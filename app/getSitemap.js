...
async function getSitemap(req, res, contentType) {
  ...
  fetch(
    ...
    .then((data) => {
      ...
      let msg = `<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://www.cheshireeast.gov.uk/${contentType.toLowerCase()}/listing</loc><lastmod>${
        data.items[0].sys.version.modified
      }</lastmod>${cf}</url>`;
